...
cross-validated performance, but this recall/precision trade-off is explicitly addressed below
via threshold tuning.
 
### Threshold Tuning (Random Forest)
 
Random Forest's default decision threshold (0.5) balances overall F1, but for a weather model
a missed rain day (false negative) is costlier than a false alarm. Since Random Forest outputs
a probability rather than a hard label, that threshold can be moved to trade some Precision for
more Recall on the Rain class:
 
| Threshold | Recall (Rain) | Precision (Rain) | F1-Score |
|-----------|----------------|-------------------|----------|
| 0.50 (default) | 0.5738 | 0.5699 | 0.5718 |
| 0.45 | 0.6388 | 0.5207 | 0.5737 |
| 0.40 | 0.7082 | 0.4789 | 0.5714 |
| **0.35 (chosen)** | **0.7647** | **0.4347** | **0.5543** |
| 0.30 | 0.8256 | 0.3949 | 0.5343 |
| 0.25 | 0.8744 | 0.3558 | 0.5058 |
 
**Chosen operating threshold: 0.35.** This raises Rain Recall from 55.95% (default threshold)
to 76.47%, at the cost of Precision dropping from 58.04% to 43.47%. The trade-off is accepted
because catching more actual rain days is prioritized over minimizing false alarms for this
use case.
